...
const Sidebar = ({ sidebarOpen, setSidebarOpen, isMember }) => {
  ...
  return (
    <ClickOutside onClick={() => setSidebarOpen(false)}>
      <aside
        className={`fixed left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-blue-500 duration-300 ease-linear lg:translate-x-0 ${
        ...
        <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
          ...
          <nav className="mt-5 px-4 lg:mt-9 lg:px-6">
            {sideBarMenus.map((group, groupIndex) => (
              <div key={groupIndex}>
                <h3 className="mb-4 ml-4 text-sm font-semibold text-white dark2">
                  {group.name}
                </h3>
